...
URL = (ROOT / "az900-practice.html").as_uri()
...
with sync_playwright() as p:
    ...
    ck("寫進 localStorage",
       q1 in pg.evaluate("localStorage.getItem('az900.star.v1')"))
    ...
    pg.evaluate("""
      ...
      Object.keys(localStorage).filter(k=>k.startsWith('az900.sess')).forEach(k=>localStorage.removeItem(k));
    ...
    ck("有寫進 localStorage",
       pg.evaluate("!!localStorage.getItem('az900.sess.v1|mine|1')"))
    size = pg.evaluate("(localStorage.getItem('az900.sess.v1|mine|1')||'').length")
    ...
    ck("壞掉的進度會從 localStorage 清掉",
       not pg.evaluate("!!localStorage.getItem('az900.sess.v1|mine|1')"))
    ck("剛開一輪沒進度時不寫檔",
       not pg.evaluate("!!localStorage.getItem('az900.sess.v1|mine|0')"))
    # 壞掉的 JSON 也不能讓程式掛掉
    pg.evaluate("localStorage.setItem('az900.sess.v1|mine|2', '{壞掉的 json')")
    pg.reload()
    pg.wait_for_function("typeof S !== 'undefined' && S.pool.length > 0")
    ...
    pg.wait_for_selector(".opts .opt")
    answer_next(pg)
    ck("有存檔", pg.evaluate("!!localStorage.getItem('az900.sess.v1|mine|1')"))
    restart(pg)
    ck("重新開始後存檔清空",
       not pg.evaluate("!!localStorage.getItem('az900.sess.v1|mine|1')"))
    pg.evaluate("S.idx = S.pool.length - 1; renderQ()")
    ck("重新開始後又有存檔", pg.evaluate("!!localStorage.getItem('az900.sess.v1|mine|1')"))
    ...
    ck("qstat 有寫進 localStorage", pg.evaluate("!!localStorage.getItem('az900.qstat.v1')"))
    pg.click("#btnCheck")            # 下一題
    ...
    ck("歷次成績有寫進 localStorage", pg.evaluate("!!localStorage.getItem('az900.hist.v1')"))
    ...
    ck("匯出檔含統計", "az900.qstat.v1" in saved["data"])
    ck("匯出檔含歷次成績", "az900.hist.v1" in saved["data"])
    ck("匯出檔含星號", "az900.star.v1" in saved["data"])
    ck("匯出檔含錯題本", "az900.wrong.v1" in saved["data"])
    ...
    # 砍掉所有資料，再匯入回來
    pg.evaluate("Object.keys(localStorage).filter(k=>k.startsWith('az900.')).forEach(k=>localStorage.removeItem(k))")
    pg.reload()
    pg.wait_for_function("typeof S !== 'undefined' && S.pool.length > 0")
