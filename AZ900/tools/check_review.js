...
console.log("=== 情境 1：文件題庫第 8 題，你選 A，正解 B ===");
const q8 = DOC.find(q => q.n === 8);
review(q8, [0], false).forEach(r =>
  console.log(`  ${r.L}  [${LABEL[r.cls].padEnd(5)}] ${r.text}  ${r.tag}`));

console.log("\n=== 情境 2：四選一，你選 C，正解 A（自製題庫）===");
const q4 = MINE.find(q => q.o.length === 4 && q.a.length === 1);
const aIdx = q4.a[0];
const wrongPick = [0, 1, 2, 3].find(i => i !== aIdx);
review(q4, [wrongPick], false).forEach(r =>
  console.log(`  ${r.L}  [${LABEL[r.cls].padEnd(5)}] ${r.text.slice(0, 30)}  ${r.tag}`));

console.log("\n=== 情境 3：複選題答對一半（需選 2 項）===");
const qm = MINE.find(q => q.a.length === 2);
