...
**已完成 9 卷、共 332 題**，寫在 `tools/bank_az104.current.js` 的 `BANK_AZ104`，已組出可直接用瀏覽器開啟的 `az104-practice.html`：
...
| Quiz11 | Q363～Q402 | 40 | ✅ 已完成，jsdom 全量驗證通過（332 題、0 error） |
| Quiz12～Quiz19 | Q403～Q711 | 308 | ⬜ 已取得來源 HTML（`source/quizN.html`），尚未轉錄 |
...
**已完成 9 卷、332 題，練習網頁可正常使用**（完整清單見第 2 節表格）。Quiz7~Quiz9 是使用者
明確指示跳過的，**不是還沒做**，不要主動回去補。Quiz11 額外處理了兩個來源資料問題，供參考：
`AZ104-Q375` 與 `AZ104-Q204`（Quiz6）是同一題但兩份來源標的正解不同（已忠實各自轉錄、解析裡
互相註明是已知分歧）；`AZ104-Q401` 的來源網頁把錯誤的選項標成正解（跟來源自己附的解析文字直接
矛盾），已依 Azure 實際知識與解析論點修正，並在該題解析裡註明原因——日後再遇到類似「正解與解析
互相矛盾」的情況，可以比照這個處理方式（修正＋註明理由），不是每次都要忠實照抄有問題的標記。

**下一卷是 Quiz12（AZ104-Q403~Q442，40 題）**。接手時**直接開始做 Quiz12**，不用再問方向、
不用再跟使用者確認要不要繼續。來源 HTML 已經在 `source/quiz12.html`，流程：

```bash
cd AZ104/tools
python extract_quiz.py ../source/quiz12.html ../source/quiz12_questions.json
```
