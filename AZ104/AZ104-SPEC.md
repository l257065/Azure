...
| 輸入 | 意思 |
|---|---|
| `17` | 本輪第 17 題（和「作答總覽」的編號一樣） |
| `2-17`／`S2-17` | 題組 2 第 17 題 |
| `T3-2` | 案例 3 第 2 題 |
| `N63`／`NewQ-63` | 增題 NewQ 第 63 題 |

解析在 `parseJump()`，`smoke.py` 有守。舊版靠 `jumpMode()` 讓「一個數字」在練習模式
代表來源題號、在考試模式代表本輪第幾題，那個雙重語意也一併拿掉了：
**一個數字永遠是本輪第幾題**，要指來源位置就寫兩段。
...
| **跳題框** | 一個數字，語意隨模式變（`jumpMode()`） | 一個數字＝本輪第幾題；`2-17`／`T3-2`／`N63`＝來源位置。`jumpMode()` 已移除 |
