...
題庫 B 的進度：題組 1 的 #1–#13 已完成（`S1#1`–`S1#13`），全部是純文字單選題。
...
3. **系列題不是重複，不能砍**：有 22 組、共 86 題屬於「同一段共用情境、每題提出不同
   `Solution:`、答案也不同」的題型（`S1#2`–`S1#4` 就是一組）。
...
但在解析裡講清楚爭議（`S1#8` 就是第一個這樣的例子）。

---

## 1-2. 題號：`sec` + `no` 兩個欄位 `[已定案]`

三份來源檔各有各的編號方式，而且 470Q 那份還分成 Question Set 1–6 與 Testlet 1–10，
**每一段的 `Question #` 都從 1 重新算**。所以**單一個號碼認不出是哪一題，一定要兩格**：
...
**為什麼不壓成一個數字。** 一開始用的是合成題號 `n = S*1000 + no`（題組 2 第 17 題 → `2017`）。
那個數字在 PDF 裡根本不存在，維護的人打開題庫檔看到 `1001` 還要心算才知道是哪一題，
而且為了它得多養一整套 encode／decode，題數說明面板還得花一段解釋編碼規則。
改成直接存兩格之後，**那些全部刪掉了**——是移除一個概念，不是多加一個。

- 欄位名不能叫 `q`：`q` 已經是題目文字。
- 引擎的 `hasSrc()` / `secName()` / `srcLabel()` / `secRank()` 負責顯示與排序，
  在 `BANK_DOC` 宣告的正下方。
- 題頭與解析標題顯示「題組 1 · 第 1 題」，英文介面顯示「Question Set 1 · #1」。
- **題庫 A（自製）沒有這兩個欄位**——它是依大綱自己編寫的、沒有來源文件。
  `hasSrc()` 為 false 的題目不顯示來源標籤、不參與跳題與缺號統計，一切照舊。
- 出題順序「依題號」用的是**檔案順序**，不是 `sec`／`no` 排序，所以照文件順序 append 就對了。

### 跳題也是兩格，對應 Question Set + Question #

```
第 3 / 13 題 · 身分與治理   [題組 1 ▾] [ 6 ]  [跳題]
                            └ 哪一段    └ 該段的 Question #
```

- **左格**（`#jumpSec`）是下拉，選項就是**這一輪實際有哪些區段**，由 `paintJump()`
  動態產生（`題組 1`、`案例 3`、`增題`…）。**只放區段，不放別的東西。**
  預設選目前這一題所屬的段，所以多數情況只要改右邊的數字。
- **右格**（`#jumpNo`）純數字，就是原始文件上印的 `Question #`。
- 這一輪一段來源座標都沒有時（例如題庫 A），左格整格 `hidden`，
  右格的數字退回當「本輪第幾題」解——維持題庫 A 原本的行為。

**為什麼要兩格。** 原本是一格，靠 `jumpMode()` 讓「一個數字」在練習模式代表來源題號、
在考試模式代表本輪第幾題——同一個輸入有兩種意思，而且從畫面上看不出來是哪一種。
中間試過一格但吃 `2-17`／`T3-2`／`N63` 這種複合寫法，問題是**使用者得先知道有這種寫法**。
拆成兩格之後 `jumpMode()` 與 `parseJump()` 都不需要了。

要跳「本輪第幾題」用**作答總覽**（點格子），跳題框不管這件事。

⚠️ **右格的 `pattern` 是踩過的坑**：一格時代它帶著 `pattern="[0-9]*"`，
打 `1-6` 會被瀏覽器的表單驗證擋在「請符合要求的格式」，連跳題邏輯都進不去。
現在右格純數字，`inputmode="numeric"` 與 `pattern` 放得回來（手機直接給數字鍵盤）。
`smoke.py` 是**端對端**驗這件事的——真的 `select_option` + `fill` + `Enter`，
而不是只呼叫函式；只驗函式那次就是漏掉了這個坑。

### 缺號只在同一段之內有意義

`docGaps()` 依 `sec` 分組，只在段內算缺號——題組 1 到 #40 就結束，題組 2 又從 #1 開始，
兩者之間沒有「缺號」可言。題數說明面板逐段列出題數與段內缺號。

### 收錄時以 470Q 為主

因為增題與 470Q 大量重疊（見 §1-1-1），同一題已經用 `S1`–`S6` 收過了，
就不要再用 `NewQ` 收一次。只有增題獨有的那些題才會出現 `sec:"NewQ"`。
...
| **題號欄位** | `n`：單一數字，就是 PDF 上的題號 | **`sec` + `no` 兩格**（`{sec:"S2", no:17}`）。AZ-104 的來源分成 Question Set／Testlet／NewQ，每段題號都從 1 重算，一個數字認不出是哪一題。見 §1-2 |
| **跳題框** | 一格數字，語意隨模式變（`jumpMode()`） | **兩格**：左邊下拉挑區段、右邊填該段的 `Question #`。`jumpMode()` 已移除 |
...
- `BANK_DOC` 的資料格式與四種題型 `mc` / `hs` / `dd` / `dl` → [AZ900-SPEC.md §5](../AZ900/AZ900-SPEC.md)
  ——**只有題號欄位不同**：AZ-900 用 `n`，AZ-104 用 `sec` + `no`（見 §1-2），其餘欄位全部相同
- 題型對應規則、洗牌規則 → §6
- 中文翻譯風格 → §7
...
tools/
  ...
  qtext.py                   倒出指定題目的原文（吃 S1-14..27 / T3-2 / NewQ-63 這種寫法）
  ...
  smoke.py                   冒煙測試：這一頁本身有沒有壞掉、來源座標與跳題解析對不對
...
### 4-2a. 純文字題：直接讀原文，不必出圖

這份 PDF 的文字抽得出來，`img` 為 0 的題目用 `qtext.py` 讀就夠了，快得多：
