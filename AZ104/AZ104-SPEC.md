...
| `BANK_DOC` | 題庫 B：文件 | `banks/bank_b_doc.js` | `AZ104考題/` 的 PDF 逐題轉錄 | **55 / 約 505（進行中）** |

題庫 B 的進度：
- **題組 1 全部 40 題完成**（`S1#1`–`S1#40`）
- **題組 2 的純文字題收了 15 題**（`S2#10`–`S2#36` 之間，跳著收）

合計 55 題，中文解析平均 693 字。

**目前的收錄策略是「先掃純文字題」**：全份還有 87 題不必出圖（扣掉 4 題重複＝實收 83），
跨區段先收完這一批，再回頭啃 343 題要出圖的。所以段內會有大量缺號，那是正常的，
`smoke.py` 的缺號檢查驗的是結構（缺號都落在存在的區段內）而不是「缺號為 0」。
...
| 領域統計 | 寫死三格 | 依 `D_MAX` 動態產生（統計面板、歷次成績的 `logRound()`、模擬考成績單的 `finishRound()` **三處都要**，見下方警語） |
...
```bash
python tools/append.py banks/bank_b_doc.js tools/batch_S1-14-27.js
...
批次檔的慣例：
- 命名 `tools/batch_<區段>-<起>-<迄>.js`，例如 `tools/batch_S1-14-27.js`。
- **開頭必須是 `,`**（因為是接在既有元素後面）。整個題庫是空的時候那一批不加逗號。
- 這些檔案是**暫存，不進版控**（`.gitignore` 只放行 `banks/` 與指定的 `tools/*`）。
  append 進題庫並驗過之後就可以刪掉——`banks/bank_b_doc.js` 才是真實來源。
- 一批大約 10–15 題比較好驗；出錯時看得出來是哪一批。
...
⚠️ **`uitest.py` 會自己跳過。** 它從頭到尾都在操作原廠題型與複選題，
要題庫同時有 `hs`、`dd`、`dl` 與複選題才跑得動；缺哪一種它會印出來然後乾淨結束
（exit 0），不是壞了。題組 1 只有 `mc` 與 `dd`，**要進題組 2 才會湊齊 `hs`／`dl`**。

在那之前，`smoke.py` 會**逐題型各畫一次**並斷言格數對得上資料
（`dd` 的答案格數、`mc` 的選項數…），所以新題型第一次進題庫就有人守著。
...
2. **PowerShell / Azure CLI / ARM / Bicep 的程式碼片段。** AZ-900 沒有這種東西。
   已確認過排版並改好引擎，寫法見 §5-1「輕量 markdown」。
...
## 5-1. 輕量 markdown：`**粗體**`、`` `程式碼` ``、``` 區塊 ``` `[已定案]`

`q` / `e` / `en.q` / `en.e` 都是純文字欄位，但寫六百字的解析很自然會用到強調與程式碼。
引擎的 `mdHtml()` 支援四種標記，**其餘一律當字面文字**：

| 寫法 | 變成 | 用在哪 |
|---|---|---|
| `⟦螢光筆⟧` | `<mark class="hl">` | 題目的關鍵條件（沿用 AZ-900） |
| `**粗體**` | `<b>` | 解析裡的判斷關鍵、結論 |
| `` `行內程式碼` `` | `<code>` | Cmdlet、參數、屬性名、題號引用（`` `S1#38` ``） |
| ` ```＋換行＋內容＋換行＋``` ` | `<pre class="code">` | 多行程式碼 |

⚠️ **多行程式碼一定要用 ``` 區塊，不要只靠換行。** 容器是 `white-space: pre-line`，
它保留換行但會把**連續空白壓成一個**——PowerShell 的續行、JSON 的層次全部會對不齊。
只有 `<pre>`（`white-space: pre`）保得住縮排。

**這是踩過的坑**：第一版直接把 markdown 寫進解析卻沒確認欄位能力，結果 900 個 `**`、
354 個反引號、2 個 ``` 區塊**全部以字面顯示**，程式碼縮排也被吃掉，34/40 題都有星號雜訊。
`mdHtml()` 就是為了修這件事加的，兩條渲染路徑（`paintHl()` DOM 版與 `escHl()` 成績單版）
共用同一份轉換，所以題目、解析、成績單的呈現一致。

守門的檢查：
- `validate.js` 擋**沒成對**的 `**`、`` ` ``、```` ``` ````（落單的標記會以字面顯示）。
  行內反引號的成對檢查會先扣掉 ``` 區塊——PowerShell 的續行符號本身就是反引號。
- `smoke.py` 驗 `paintHl()` 真的產出 `<b>`／`<code>`／`<pre>`／`<mark>`、程式碼區塊的縮排有留住、
  畫面上不留字面的 `**` 與 ```，以及**落單的標記要原樣留著、不能吃掉後面的字**。

---

## 5-2. 拖放題（`dd`）的兩種答案區 `[已定案]`
...
**二、建立清單（build list）** — 答案區是**沒有標籤的 N 個空格**，只有上下順序。
`tgt` 用 `["第 1 個","第 2 個",…]`（英文 `["Slot 1",…]`）並**設 `fix:true`**，
答案區不洗、只洗可拖曳的項目。AZ-900 的 `#162`／`#467` 就是這樣處理的。

題組 1 的兩題都屬於第二種：`S1#20`（Key Vault ＋ 存取原則）、`S1#23`（Site Recovery 三個物件）。

⚠️ **順序是不是答案的一部分，要在解析裡講清楚。** 這兩題原文問的都是「要建立哪些物件」，
是**集合**而不是順序，答案區的空格沒有標籤；但本引擎是**逐格批改**，所以只能照原始文件圖上的
上下順序排。解析末尾要寫明這件事，`S1#23` 還額外註明實務上的建立順序與圖上不同
（實際要先建保存庫），**叫人記概念不要記順序**。
...
「同一段共用情境、每題提出不同 `Solution:`」的系列題共 **22 組 86 題**（見 §1-1-1），
題組 1 就有 6 組。慣例：

- **每一題都完整收錄**，共用情境照抄進每一題的 `q`，不共用、不省略。它們是不同的題目。
- 第一段用 `注意：本題與另外數題的情境完全相同，但每一題提出的解法不同，請個別判斷解法是否達成目標。`
  （渲染成 `.qnote` 小字灰底，與題目分開）；PDF 用較長樣板的那些（`S1#38`–`S1#40`）
  照原文譯出「回答後不能再回頭修改」那一段。
- **`解法：…` 自成一段並整段標 `⟦⟧`**，對應 PDF 的排版，也讓人一眼看到差異點在哪。
- 解析裡**互相引用同組的其他題**，用 `` `S1#38` `` 這種來源座標，例如
  「這一組三個解法對照：… `S1#39` 也是建成員 …」。這樣三題讀起來是一組完整的知識，
  而不是三段重複。
- 整組**只有一題是「是」**是常態，但原廠樣板明說也可能全部都不是。
  `dupes.py --series` 會列出「整組都標 No」的組別，那種要回頭確認（見 §1-1-1）。
...
- [x] ~~題號要不要拆~~ → 拆成 `sec` + `no` 兩格，合成題號 `n` 已廢除（§1-2）
- [x] ~~程式碼片段的排版~~ → 引擎加了輕量 markdown，程式碼走 `<pre>` 才保得住縮排（§5-1）
- [x] ~~拖放題無標籤答案區怎麼寫~~ → `第 1 個／第 2 個` ＋ `fix:true`（§5-2）
- [x] ~~系列題怎麼寫~~ → 每題完整收錄、解法整段標記、解析互相引用（§5-3）
- [ ] **接著收題組 2**（79 題，`hs` 23／`dd` 3／`mc` 53；只有 22 題可純文字轉錄，
      57 題要 `render.py` 出圖）。收到 `hs`／`dl` 之後 `uitest.py` 才會開始跑（§4-5）
- [ ] 補 AZ-104 專屬術語進 `GLOSSARY_MAP`（§2）
- [ ] 遇到第一題 case study（Testlet）時，問使用者要怎麼處理（§5，第 4 點）
- [ ] 問使用者 `AZ-104-26.95-Correct.txt` 的來源，它的題號對不上這份 PDF（§1-1-1）
- [ ] 題組 4 的 #43／#81 等 **14 組答案矛盾的重複題**，收到時要出圖人工裁決（§1-1-1）
...
### ⚠️ 寫死「三個領域」是最容易漏的一類 bug

AZ-900 有 3 個領域、AZ-104 有 5 個，`build_skeleton.py` 換殼時漏了兩處：

| 位置 | 症狀 | 什麼時候才會發現 |
|---|---|---|
| `logRound()` 的 `byD` | 領域 4／5 的成績被**靜靜丟掉**（有 `if(d)` 保護所以不報錯） | 幾乎不會，除非去對歷次成績的數字 |
| `finishRound()` 的 `byD` | `byD[4]` 是 `undefined` → **整張成績單爆掉**，交卷後畫面停住 | 題庫收到第一題領域 4／5 且被模擬考抽到時 |

兩處都已改成用 `blankByD()` 依 `D_MAX` 產生。這個 bug 直到題組 1 收完
（第一次出現領域 4／5 的題目）才浮出來——**題庫只有領域 1–3 時完全看不出來**。

守門的檢查：
- `validate.js` 擋 `d` 超出 1–5 的題目
- `smoke.py` 交一次模擬考，斷言成績單的領域細項剛好 `D_MAX` 格、逐題檢討筆數等於抽題數
- `check_star_flag.py` 斷言歷次成績的 `hist[0].d` 有 `D_MAX` 個鍵

日後若引擎再從 AZ-900 換殼過來，**先搜 `{1:[0,0]` 這種寫死的字樣**。
...
| `d` | 收哪些 | 邊界案例 |
|---|---|---|
| `1` 身分與治理 | Entra ID／Azure AD 使用者群組、RBAC 角色指派、條件式存取、MFA、AD Connect 同步、訂用帳戶與管理群組、資源群組、標記、Azure Policy、成本管理、資源鎖 | 「用 Azure Policy 限制儲存體帳戶的 SKU」→ **`1`**（在問 Policy 怎麼運作）<br>「儲存體帳戶要選哪個備援等級」→ `2` |
...
| `3` 運算資源 | **ARM 範本與 Bicep（判讀、修改、部署、匯出、部署歷程）**、虛擬機器與磁碟、可用性設定組與可用性區域、擴展集、VM 擴充功能、映像與快照、App Service、容器實例與 Azure Container Apps、AKS | 「VM 的網路介面要接哪個子網」→ `4`（在問網路）<br>「VM 大小改不了怎麼辦」→ `3`<br>**「去哪裡看某次部署用的 ARM 範本」→ `3`**（不是 `1`；官方大綱把 ARM／Bicep 放在運算） |
...
⚠️ **ARM 範本與 Bicep 是 `3` 不是 `1`。** 直覺會想歸「治理」，但
[AZ104-EXAM-TDD.md](AZ104-EXAM-TDD.md) §1-2 的官方功能群組把
「ARM 範本與 Bicep（判讀、修改、部署、匯出與轉換）」放在
**Deploy and manage Azure compute resources**。第一版的判定表寫錯過一次，
`S1#13`／`S1#14`／`S1#15`／`S1#20` 都因此改成 `d:3`。
