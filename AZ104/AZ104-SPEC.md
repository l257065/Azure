...
| `BANK_DOC` | 題庫 B：文件 | `banks/bank_b_doc.js` | `AZ104考題/` 的 PDF 逐題轉錄 | **40 / 約 505（進行中）** |

題庫 B 的進度：**題組 1 全部 40 題完成**（`S1#1`–`S1#40`）。
題型 `mc` 38 題（含 1 題複選）＋ `dd` 2 題；領域分布 1:13／2:1／3:11／4:10／5:5。
中文解析平均 669 字。下一段是題組 2（79 題，其中 57 題要出圖）。
