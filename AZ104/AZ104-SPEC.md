...
| `BANK_DOC` | 題庫 B：文件 | `banks/bank_b_doc.js` | `AZ104考題/` 的 PDF 逐題轉錄 | **13 / 470（進行中）** |
...
| 檔案 | 頁數 | 題數 | 答案來源 |
|---|---|---|---|
| `NEW-AZ-104-470Q.pdf` | 759 | 470 | 文件內自帶 `Correct Answer:` |
| `New-AZ-104-增題62Q.pdf` | 62 | 待掃 | 同上 |
| `NewQuestion-AZ-104-NewQ63-Q76.pdf` | 13 | 待掃 | `Q63-Q76 Correct.txt` |
...
這份 PDF 分成 Question Set 1–6 與 Testlet 1–10，**每一段的 `Question #` 都從 1 重新算**，
直接拿 `#N` 當 `n` 會撞號。所以把區段編進題號：

| 來源 | `n` | 範圍 |
|---|---|---|
| Question Set `S` 第 `q` 題 | `S*1000 + q` | 1001…6047 |
| Testlet `T` 第 `q` 題 | `10000 + T*100 + q` | 10101…11004 |

`2017` 就是題組 2 第 17 題，`10302` 就是案例 3 第 2 題。這樣 `n` 仍然是數字
（跳題功能要數字比對），而且看得懂、回原始文件查得到。
